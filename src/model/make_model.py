...
class CCCU(nn.Module):
    """
    上下文耦合校准单元 (Context-Coupled Calibration Unit)
    用于计算两个特征 (a, b) 之间的耦合权重，并对特征 a 进行重新校准
    """
    def __init__(self, in_dim):
        super(CCCU, self).__init__()
        # phi 和 psi 对应公式中的线性映射
        self.phi_a = nn.Linear(in_dim, in_dim, bias=False)
        self.phi_b = nn.Linear(in_dim, in_dim, bias=False)
        self.psi_a = nn.Linear(in_dim, in_dim, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, a, b):
        # a 和 b 的 shape 均为 [batch_size, dim]
        # phi(a) ⊙ phi(b): 使用逐元素相乘 (element-wise product)
        fusion = self.phi_a(a) * self.phi_b(b)
        
        # σ(phi(a) ⊙ phi(b)): 得到维度上的门控权重
        attention_weight = self.sigmoid(fusion)
        
        # 乘以 ψ(a) 完成校准
        calibrated_feat = attention_weight * self.psi_a(a)
...
class build_transformer_local(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory, rearrange):
        ...
        # 初始化 CCCU 模块：包含第一阶段(全局到局部)和第二阶段(局部到全局)
        self.cccu_step1 = nn.ModuleList([CCCU(self.in_planes) for _ in range(4)])
        self.cccu_step2 = nn.ModuleList([CCCU(self.in_planes) for _ in range(4)])
    ...
    def forward(self, x, x2, x3, label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        # 【阶段一：全局引导局部】 
        # 公式: p_tilde_k = p_k + C(p_k, z_o)
        # 以全局语义作为补充信息，对局部细节进行上下文校准
        local_feat_1 = local_feat_1 + self.cccu_step1[0](local_feat_1, global_feat_ori)
        local_feat_2 = local_feat_2 + self.cccu_step1[1](local_feat_2, global_feat_ori)
        local_feat_3 = local_feat_3 + self.cccu_step1[2](local_feat_3, global_feat_ori)
        local_feat_4 = local_feat_4 + self.cccu_step1[3](local_feat_4, global_feat_ori)

        # 【阶段二：局部反哺全局】
        # 公式: g^(k) = g^(k-1) + C(g^(k-1), p_tilde_k)
        # 递进式聚合：用校准后的局部细节反向更新全局表示
        g0 = global_feat_ori
        g1 = g0 + self.cccu_step2[0](g0, local_feat_1)
        g2 = g1 + self.cccu_step2[1](g1, local_feat_2)
        g3 = g2 + self.cccu_step2[2](g2, local_feat_3)
        global_feat = g3 + self.cccu_step2[3](g3, local_feat_4)
